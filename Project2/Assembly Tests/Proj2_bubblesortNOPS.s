...
whileloop:
	...
	nop
	nop
	nop		#3 nops because we need to know if we are branching or not before loading the next instruciton from pc+4 or the new address.
...
forloop:
	...
	nop
	nop			#3 nops  to allow $t1 to finish loading
	nop
	
	bgt $t0, $t1, swap	#if numbers in i > numbers in i+1, need to sort.
	
	nop
	nop			#3 nops to allow us to not load bad data if we didnt branch.
	nop
	...
	nop
	nop
	nop			#3 NOPS TO NOT LOAD BAD DATA BECAUSE OF JUMP, we want to wait until the jump adress is given back to the pc.
...
increment:	
	...
	nop
	nop
	nop			#3 nops to allow s1 to write.
	...
	nop
	nop			#3 nops to allow delay for jumping
	nop
